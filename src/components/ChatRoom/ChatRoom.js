...
const ChatRoom = () => {
  ...
  useEffect(() => {
    ...
    const wsServerConnection = new ReconnectingWebSocket(
      `wss://${backendAddr}/ws`,
      null,
      optionsWebsocket
    );
    ...
    wsServerConnection.onclose = (e) => {
      ...
      if (websocketConnectionClosedIdx % numClosedWebsocketForWarning === 0) {
        alert(
          "1st Your internet connection seems to be unstable. The service has trouble connecting the server."
        );
      }
    };
    ...
    wsServerConnection.onerror = (e) => {
      ...
      if (websocketConnectionErrorIdx % numErrorsWebsocketForWarning === 0) {
        alert(
          "2nd Your internet connection seems to be unstable. The service has trouble connecting the server."
        );
      }
    };
